...
export default function MapScreen() {
  ...
  async function recalculateRoute() {
    ...
    try {
      ...
      const result = await getRoute(location.latitude, location.longitude, destination.latitude, destination.longitude, { mode });
      if (!result) return;
  ...
  async function searchDestination(query) {
    ...
    try {
      ...
      const result = await getRoute(location.latitude, location.longitude, destination.latitude, destination.longitude, { mode });
      if (!result) {
        Alert.alert('CrossNav', 'Nie znaleziono trasy dla wybranego trybu.');
        return;
      }
      setRoute({ ...result, destination });
      setStepIndex(0);
  ...
  if (!location) return <View style={styles.center}><Text style={styles.loading}>📡 Pobieranie GPS...</Text></View>;

  return (
    <View style={styles.container}>
      <WebView ref={webViewRef} originWhitelist={['*']} source={{ html: getMapHtml(location.latitude, location.longitude, route) }} style={StyleSheet.absoluteFill} javaScriptEnabled domStorageEnabled />
