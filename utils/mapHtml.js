export function getMapHtml(latitude, longitude, route = null) {
  ...
  const routeJson = JSON.stringify(routes.map((item, index) => ({
...
function routeLabel(i){if(i===0)return '🥇 Najszybsza';return '🛣️ Alternatywna '+i}
...
function renderRoutes(){
 ...
 routeData.forEach((r,i)=>{if(!r.geometry?.length)return;const pts=r.geometry.map(p=>[p[1],p[0]]);const line=L.polyline(pts,{color:i===selectedRoute?'#1976d2':'#777',weight:i===selectedRoute?7:4,opacity:i===selectedRoute?.9:.55}).addTo(map);line.on('click',()=>{selectedRoute=i;renderRoutes()});routeLines.push(line)});
 const panel=document.getElementById('routePanel');if(!routeData.length){panel.style.display='none';return}panel.style.display='block';panel.innerHTML=routeData.map((r,i)=>`<div class="route-option ${i===selectedRoute?'active':''}" data-route="${i}">${routeLabel(i)}<div class="route-time">${formatDuration(r.duration)} · ${formatKm(r.distance)}</div><div class="route-meta">Kliknij, aby wybrać tę trasę</div></div>`).join('');panel.querySelectorAll('.route-option').forEach(el=>el.onclick=()=>{selectedRoute=Number(el.dataset.route);renderRoutes()});
