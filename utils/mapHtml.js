...
#gpsInfo {
  ...
  font-size:17px; font-weight:bold; box-shadow:0 2px 8px rgba(0,0,0,.25);
}
#direction {
  position:absolute; top:70px; left:50%; transform:translateX(-50%);
  z-index:9999; width:58px; height:58px; border-radius:50%;
  background:white; display:flex; align-items:center; justify-content:center;
  font-size:34px; box-shadow:0 2px 8px rgba(0,0,0,.25);
}
.crossnav-motorcycle { background:transparent !important; border:0 !important; }
.crossnav-motorcycle-inner {
  width:40px; height:40px; display:flex; align-items:center; justify-content:center;
  font-size:34px; transform-origin:center center;
...
const map = L.map('map').setView([startLat, startLon], 15);
...
if (routeCoordinates.length) {
  ...
  const routeLine = L.polyline(points, {
    color:'#1976d2', weight:6, opacity:.85
  }).addTo(map);
  try { map.fitBounds(routeLine.getBounds(), { padding:[40,40] }); } catch(e) {}
}

const motorcycleIcon = L.divIcon({
  className:'crossnav-motorcycle',
  html:'<div id="motorcycle" class="crossnav-motorcycle-inner">🏍️</div>',
...
window.updateCrossNavGPS = function(message) {
  try {
    ...
    if (typeof data.speed === 'number' && data.speed >= 0) {
      speedText = '🏍️ ' + Math.round(data.speed * 3.6) + ' km/h';
    }
    ...
    if (typeof data.heading === 'number' && data.heading >= 0) {
      ...
      if (direction) {
        direction.style.transform = 'translateX(-50%) rotate(' + heading + 'deg)';
